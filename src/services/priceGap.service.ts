 ...
 *
 * gap은 close_gap으로, 가격 3종은 close_kr_price/close_usd_ref/close_ex_price(0011 추가)로
 * 복원한다. 0011 이전에 적재된 구 행은 가격 컬럼이 null이라 그 분이 마지막이면 가격은 null로
 * 내려간다(프론트는 기존 null→— 처리). 완전한 종료값은 정상 운영(메모리 스냅샷)이 보장하고,
 * 이건 재배포 예외의 차선책이다.
 *
...
export async function closingRowsFromOhlc(fromIso: string): Promise<GapSnapshotRow[]> {
  const { data, error } = await supabase
    .from('price_gap_ohlc')
    .select('timestamp_minute, stock_code, stock_name, exchange, close_gap, close_kr_price, close_usd_ref, close_ex_price')
  ...
  const latest = new Map<string, GapSnapshotRow>();
  for (const r of data ?? []) {
    const key = `${r.exchange}:${r.stock_code}`;
    latest.set(key, {
      stockCode: r.stock_code as string,
      stockName: r.stock_name as string,
      exchange: r.exchange as GapExchange,
      krPrice: (r.close_kr_price as number | null) ?? null,
      usdRef: (r.close_usd_ref as number | null) ?? null,
      exPrice: (r.close_ex_price as number | null) ?? null,
      gap: r.close_gap as number,
